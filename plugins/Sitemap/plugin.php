...
add_plugin_hook('define_routes', 'xml_sitemap_routes');
...
function xml_sitemap_routes($args)
{           
        $router = $args['router'];
	$router->addRoute(
	    'xml_sitemap', 
...
function xml_sitemap_admin_nav($nav)
{
    //$navArray = $navArray + array('Xml Sitemap' => url(array('module'=>'xml-sitemap', 'controller' => 'index', 'action'=>'index'), 'default'));
     $nav[] = array(
            'label' => __('Xml Sitemap'),
            'uri' => url('xml-sitemap')
        );
    return $nav;
}
...
// define function to actually create the site_map.xml file  
function xml_sitemap_build_html_sitemap()
{
    $html = '';
    $childPageLinks = simple_pages_get_links_for_children_pages(0, 'order', true);
    if ($childPageLinks) {
        $html .= '<div class="sitemap-links">' . "\n";
        ...
        $html .= '</ul></li></ul>';
        //contact
        $html .= '<ul class="navigation"><li>';
        $html .= '<a href="'.url("contact").'">Contact</a>';        
        $html .= '</li></ul>';
...
function xml_sitemap_set_freq($contentID, $contentLabel) {
	...
	//$field .= select(, $frequencyChoices, get_option($contentID), $contentLabel);
	//select($attributes, $values = null, $default = null, $label=null, $labelAttributes = array())
        //formSelect($name, $value = null, $attribs = null,$options = null, $listsep = "<br />\n")
        $field .= get_view()->formSelect($contentID, $contentLabel, array('name' => $contentID, 'id' => $contentID),$frequencyChoices);
        $field .= "</div>";
	return $field;
...
function xml_sitemap_set_rank($rankID, $rankLabel) {
	...
	//$field .= get_view()->formSelect(array('name' => $rankID, 'id' => $rankID), $rankingChoices, get_option($rankID), $rankLabel);
	$field .= get_view()->formSelect($rankID, $value = null, array('name' => $rankID, 'id' => $rankID),$rankingChoices);
        $field .= "</div>";
	return $field;
}
